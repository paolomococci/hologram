<div class="m-4">
    ...
    <form class="flex justify-center w-9/12">
        <button
            ...
            wire:click.prevent="toggle()" {{ empty($filterText) ? 'disabled' : '' }} wire:offline.attr="disabled">
        ...
        <button class="p-4 my-0 text-white bg-green-600 rounded-r-md border disabled:bg-slate-400 hover:bg-red-600"
            wire:click.prevent="clear()" {{ empty($filterText) ? 'disabled' : '' }} wire:offline.attr="disabled">
            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none"
                stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
