# `node-app-1.0` how to customize an image named `node-app-img:1.0`
...
```bash
ls ~/projects/containers/frontend/node-app-1.0
cd ~/projects/containers/frontend/node-app-1.0
```
...
podman image list
cat Dockerfile
podman build --tag node-app-img:1.0 .
```
...
```bash
podman images --all
podman images --no-trunc --quiet node-app-img:1.0
podman image inspect node-app-img:1.0
```
...
podman run --volume $(pwd)/html:/var/www/html --detach --name node-app-cntr-1-0 --publish 5173:5173 --publish 8080:80 --publish 8443:443 --publish 8022:22 --pull=never  node-app-img:1.0
podman container list --all --size
podman exec --interactive --tty --privileged node-app-cntr-1-0 bash
```
...
```bash
podman stop node-app-cntr-1-0
```

### restart the container

I can proceed to restarting `node-app-cntr-1-0` in privileged mode:

```bash
podman container list --all
podman start node-app-cntr-1-0
podman exec --interactive --tty --privileged node-app-cntr-1-0 bash
```
...
```bash
podman stop node-app-cntr-1-0 && podman rm node-app-cntr-1-0
podman container list --all
```
...
```bash
podman image rm node-app-img:1.0
podman image list
```
