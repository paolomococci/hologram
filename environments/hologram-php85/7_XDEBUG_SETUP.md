...
```shell
ln --symbolic --verbose /opt/php/8.5.0/bin/php /usr/bin/php
ln --symbolic --verbose /opt/php/8.5.0/bin/phar.phar /usr/bin/phar
ln --symbolic --verbose /opt/php/8.5.0/bin/phpize /usr/bin/phpize
ln --symbolic --verbose /opt/php/8.5.0/bin/php-config /usr/bin/php-config
exit
```
...
sudo updatedb
locate xdebug.ini
rnano /opt/php/8.5.0/lib/php.ini
```

First it is a good idea to view the contents of the file without risking causing damage.
And now I edit `/opt/php/8.5.0/lib/php.ini` configuration file

```shell
sudo nano /opt/php/8.5.0/lib/php.ini
```
...
Start debugging from `vscode` and, at the same time, point to address <https://192.168.1.85/sample.php> from the browser.
