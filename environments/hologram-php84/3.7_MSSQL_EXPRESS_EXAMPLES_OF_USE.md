...
```shell
sqlcmd -S 127.0.0.1 -U newly_username -N -C
```

Now, if you want, you can practice with the following SQL commands:

```sql
-- could you check which database you are in
select db_name();
go
...
-- to then check that the schema has been created correctly
...
-- if you want to create a function that checks the validity of the value passed to the Deadline field of the Projects table
create function ExampleV1Sc.CheckEffectiveDeadlineFn (@Deadline INT) returns int as begin return case when @Deadline is null then 1 when @Deadline > 0 then @Deadline else 1 end; end;
...
create procedure ExampleV1Sc.InsertProject @TitleProject nvarchar(256), @DescriptionProject nvarchar(max), @IsUrgent bit, @IsDone bit, @IsCanceled bit, @IntentId bigint, @UserId bigint, @Deadline int as begin insert into ExampleV1Sc.Projects (TitleProject, DescriptionProject, IsUrgent, IsDone, IsCanceled, IntentId, UserId, Deadline) values (@TitleProject, @DescriptionProject, @IsUrgent, @IsDone, @IsCanceled, @IntentId, @UserId, dateadd(day, ExampleV1Sc.CheckEffectiveDeadlineFn(@Deadline), getdate())) end;
go
...
exec ExampleV1Sc.InsertProject @TitleProject = 'Bug Fix sample four', @DescriptionProject = 'Fix critical bug in the sample four', @IsUrgent = 0, @IsDone = 0, @IsCanceled = 1, @IntentId = 1, @UserId = 4, @Deadline = NULL;
go

exec ExampleV1Sc.InsertProject @TitleProject = 'Bug Fix sample five', @DescriptionProject = 'Fix critical bug in the sample five', @IsUrgent = 0, @IsDone = 0, @IsCanceled = 1, @IntentId = 1, @UserId = 4, @Deadline = -6;
go

exec ExampleV1Sc.InsertProject @TitleProject = 'Bug Fix sample six', @DescriptionProject = 'Fix critical bug in the sample six', @IsUrgent = 0, @IsDone = 0, @IsCanceled = 1, @IntentId = 1, @UserId = 4, @Deadline = 7;
go
...
-- or even better
select IdProject as Id, TitleProject as Title, Deadline from ExampleV1Sc.Projects;
go
...
-- if you want to ensure that all records have a positive Deadline at the table level, you could add a check constraint
alter table ExampleV1Sc.Projects alter column Deadline datetime2 not null;
go

alter table ExampleV1Sc.Projects with nocheck add constraint Project_Deadline_Positive_CK check ((Deadline >= dateadd(day, 1, getdate())));
go
```
...
-- or even better
select IdProject as Id, TitleProject as Title, Deadline from ExampleV1Sc.ProjectsExpiringView order by Deadline;
go
```
