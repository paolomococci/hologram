...
../configure --prefix=/opt/php/8.4.8 --enable-fpm --enable-bcmath --enable-ftp --with-openssl --disable-cgi --enable-mbstring --with-curl --with-mysqli --with-pdo-mysql --enable-intl --with-zlib --with-bz2 --enable-gd --with-jpeg --with-gettext --with-gmp --with-xsl --enable-zts --enable-gcov --enable-debug --with-ffi --with-zip
make
make test
sudo make install
cd ~
...
sed -i 's/memory_limit = 128M/memory_limit = 256M/g' /opt/php/8.4.8/lib/php.ini
grep -i "max_execution_time" /opt/php/8.4.8/lib/php.ini
sed -i 's/max_execution_time = 30/max_execution_time = 100/g' /opt/php/8.4.8/lib/php.ini
grep -i "upload_max_filesize" /opt/php/8.4.8/lib/php.ini
sed -i 's/upload_max_filesize = 2M/upload_max_filesize = 8M/g' /opt/php/8.4.8/lib/php.ini
...
```bash
grep -i "short_open_tag = Off" /opt/php/8.4.8/lib/php.ini
...
At the end of the `/opt/php/8.4.8/etc/php-fpm.conf` file

```bash
nano /opt/php/8.4.8/etc/php-fpm.conf
```
...
```bash
grep -i "zend_extension" /opt/php/8.4.8/lib/php.ini
