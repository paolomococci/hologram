...
```bash
ln --symbolic --verbose /opt/php/8.4.11/bin/php /usr/bin/php
ln --symbolic --verbose /opt/php/8.4.11/bin/phar.phar /usr/bin/phar
ln --symbolic --verbose /opt/php/8.4.11/bin/phpize /usr/bin/phpize
ln --symbolic --verbose /opt/php/8.4.11/bin/php-config /usr/bin/php-config
exit
```
...
sudo updatedb
locate xdebug.ini
rnano /opt/php/8.4.11/lib/php.ini
```

First it is a good idea to view the contents of the file without risking causing damage.
And now I edit `/opt/php/8.4.11/lib/php.ini` configuration file

```bash
sudo nano /opt/php/8.4.11/lib/php.ini
```
