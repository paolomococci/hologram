    ...
    <div class="mx-auto w-full max-w-xl">
        ...
        <div role="search"
            ...
            <input type="search" {{-- Custom placeholder text passed from the component --}} placeholder="{{ $placeholder }}" {{-- Accessibility label for screen readers --}}
                aria-label="Search" class="flex-1 px-4 py-2 w-full border-0 focus:outline-none focus:ring-0"
            ...
            <button type="button" {{-- Calls the "erase" method on the Livewire component when clicked --}} wire:click="erase" {{-- Accessibility label --}} aria-label="Clear"
                ...
                {{-- Small SVG glyph component that displays an icon_name icon --}}
                {{-- remember to replace the icon_name placeholder with the correct name of your SVG file --}}
                <x-glyph name="icon_name" title="To clear the search field" />
            </button>
        </div>
