# `lamp-pre-build-1.3` how to customize an image named `lamp-pre-build-img:1.3`
...
```bash
ls ~/projects/containers/backend/lamp-pre-build-1.3
cd ~/projects/containers/backend/lamp-pre-build-1.3
```
...
podman image list
cat Dockerfile
podman build --tag lamp-pre-build-img:1.3 .
```
...
```bash
podman images --all
podman images --no-trunc --quiet lamp-pre-build-img:1.3
podman image inspect lamp-pre-build-img:1.3
```
...
podman run --volume $(pwd)/html:/var/www/html --detach --name lamp-pre-build-cntr-1-3 --publish 8080:80 --publish 8443:443 --publish 8022:22 --pull=never  lamp-pre-build-img:1.3
podman container list --all --size
podman exec --interactive --tty --privileged lamp-pre-build-cntr-1-3 bash
```
...
```bash
podman stop lamp-pre-build-cntr-1-3
podman container list --all
```
...
```bash
podman container list --all
podman start lamp-pre-build-cntr-1-3
podman exec --interactive --tty --privileged lamp-pre-build-cntr-1-3 bash
```
...
```bash
podman stop lamp-pre-build-cntr-1-3 && podman rm lamp-pre-build-cntr-1-3
podman container list --all
```
...
```bash
podman image rm lamp-pre-build-img:1.3
podman image list
```
