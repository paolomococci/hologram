...
cd ~/projects/apps/agora/agora-1.0/frontend/vue-flavor-cntr-1.0
```
...
podman run --volume $(pwd)/html:/var/www/html --detach --name vue-flavor-cntr-1-0 --publish 5173:5173 --publish 8080:80 --publish 8443:443 --publish 8022:22 --pull=never node-app-img:1.0
podman container list --size
podman exec --interactive --tty --privileged vue-flavor-cntr-1-0 bash
```
...
```bash
podman stop vue-flavor-cntr-1-0
```

### restart the container

I can proceed to restarting `vue-flavor-cntr-1-0` in privileged mode:

```bash
podman container list --all
podman start vue-flavor-cntr-1-0
podman exec --interactive --tty --privileged vue-flavor-cntr-1-0 bash
```
...
```bash
podman stop vue-flavor-cntr-1-0 && podman rm vue-flavor-cntr-1-0
```
