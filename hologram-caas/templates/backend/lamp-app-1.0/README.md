# `lamp-app-1.0` how to customize an image named `lamp-app-img:1.0`
...
```bash
ls ~/project/templates/backend/lamp-app-1.0
cd ~/project/templates/backend/lamp-app-1.0
```
...
```bash
docker image ls | grep "lamp-app-img"
cat Dockerfile
docker build --tag lamp-app-img:1.0 .
```
...
```bash
docker images --all
docker images --no-trunc --quiet lamp-app-img:1.0
docker image inspect lamp-app-img:1.0
```
...
docker run --volume $(pwd)/html:/var/www/html --detach --name lamp-app-cntr-1-0 --publish 8022:22 --publish 8080:80 --publish 8443:443 --publish 9003:9003 --pull=never  lamp-app-img:1.0
docker container ls --all --size
docker exec --interactive --tty --privileged lamp-app-cntr-1-0 bash
```
...
```bash
docker container ls --all
docker start lamp-app-cntr-1-0
docker exec --interactive --tty --privileged lamp-app-cntr-1-0 bash
```
...
```bash
docker stop lamp-app-cntr-1-0 && docker rm lamp-app-cntr-1-0
docker container ls --all
```
...
```bash
docker image rm lamp-app-img:1.0
docker image ls
```
